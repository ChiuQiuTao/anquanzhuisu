
<template>
    <section class="product-search">
        <Row>
            <i-col span="24" class="search-title">查询条件</i-col>
        </Row>
        <br>
        <Row>
            <i-col span="24" class="type">
                <section style="margin-bottom:18px;margin-right:60px;">
                    <section class="type-title">
                        产品类型
                    </section>
                    <section class="type-select">
                        <i-select :model.sync="model1" style="width:160px;" class="type-select-c">
                            <i-option v-for="item in list" :value="item.value">{{ item.label }}</i-option>
                        </i-select>
                    </section>
                </section>



                <section style="margin-bottom:18px;margin-right:60px;">
                    <section class="type-title">
                        企业名称
                    </section>
                    <section class="type-select">
                        <row>
                            <i-input :value.sync="value" placeholder="输入企业名称" style="width: 150px" class="type-select-c"></i-input>
                        </row>
                    </section>
                </section>


                <section style="margin-bottom:18px;margin-right:60px;">
                    <section class="type-title">
                        产品名称
                    </section>
                    <section class="type-select">
                        <row>
                            <i-input :value.sync="value" placeholder="输入产品名称" style="width: 150px" class="type-select-c"></i-input>
                        </row>
                    </section>
                </section>


                <section style="margin-bottom:18px;margin-right:60px;">
                    <section class="type-title">
                        追溯码
                    </section>
                    <section class="type-select">
                        <row>
                            <i-input :value.sync="value" placeholder="输入追溯码" style="width: 150px" class="type-select-c"></i-input>
                        </row>
                    </section>
                </section>

                <section class="type-btn" style="margin-bottom:18px;">
                    <i-button type="primary" shape="circle" class="type-btn-c" size="small">查询</i-button>
                    <i-button type="primary" shape="circle" class="type-btn-c" size="small">重置</i-button>
                </section>
            </i-col>
        </Row>
    </section>
    
    
</template>
<script>
import './index.less'

    export default {
        data(){
            return{
                list:[
                    {
                        value:'投入品保质期预警',
                        label:'投入品保质期预警'

                    },{
                        value:'投入品保质期预警',
                        label:'投入品保质期预警'

                    },
                ]
            }
        }
    }
</script>

<style>

</style>
